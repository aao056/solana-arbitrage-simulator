...
pub fn derive_oracle_pda(lb_pair: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[ORACLE, lb_pair.as_ref()], &meteora_dlmm::ID)
}
...
pub fn derive_reserve_pda(token_mint: Pubkey, lb_pair: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[lb_pair.as_ref(), token_mint.as_ref()], &meteora_dlmm::ID)
}
...
pub fn derive_event_authority_pda() -> (Pubkey, u8) {
    Pubkey::find_program_address(&[b"__event_authority"], &meteora_dlmm::ID)
}
...
pub fn derive_token_badge_pda(mint: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[TOKEN_BADGE, mint.as_ref()], &meteora_dlmm::ID)
}
