...
impl LbPairExtension for LbPair {
    ...
    fn next_bin_array_index_with_liquidity_internal(
        ...
        let start = start_array_index.clamp(min_bitmap_id, max_bitmap_id);

        let is_set = |idx: i32| -> Result<bool> {
            let offset = Self::get_bin_array_offset(idx);
            let limb = offset.checked_div(64).context("overflow")?;
            let bit = offset.checked_rem(64).context("overflow")?;
            let word = *self.bin_array_bitmap.get(limb).context("overflow")?;
            Ok(((word >> bit) & 1) == 1)
        };

        if swap_for_y {
            let mut idx = start;
            while idx >= min_bitmap_id {
                if is_set(idx)? {
                    return Ok((idx, true));
                }
                idx = idx.checked_sub(1).context("overflow")?;
            }
            Ok((min_bitmap_id.checked_sub(1).context("overflow")?, false))
        } else {
            let mut idx = start;
            while idx <= max_bitmap_id {
                if is_set(idx)? {
                    return Ok((idx, true));
                }
                idx = idx.checked_add(1).context("overflow")?;
            }
